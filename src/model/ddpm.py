...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            ...
            lspace = torch.linspace(self.beta_start, self.beta_end, self.T).to(self.device) # to(self.device) is added from chat
            return lspace 
        elif schedule == 'cosine':
            # add your own (e.g. cosine)
            # HINT: use torch.cosine_similarity to create a cosine schedule from beta_start to beta_end
            torch.cosine_similarity(self.beta_start, self.beta_end, self.T)        
            return cspace                       
        else :
    ...
    #    if schedule == 'linear':
    #        # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
    #        lspace = torch.linspace(1-self.beta_start, 1-self.beta_end, self.T)
    #        return lspace 
    #    elif schedule == 'cosine':
    #        # add your own (e.g. cosine)
    #        # HINT: use torch.cosine_similarity to create a cosine schedule from beta_start to beta_end
    #        torch.cosine_similarity(1-self.beta_start, 1-self.beta_end, self.T)        
    #        return cspace                       
    #    else :
    #        raise NotImplementedError('Not implemented!')
    
        """
        q_sample: sample from q(x_t | x_0)
    ...
    def q_sample(self, x, t):
        ...
        x_noised = x * sqrt_alpha_bar + sqrt_one_minus_alpha_bar * noise # HINT: Create the noisy version of x. See Eq. 4 in the ddpm paper at page 2
    ...
    def p_sample(self, model, x_t, t):
        ...
        if t[0]>1: # HINT: Having calculate the mean and std of p(x{x_t} | x_t), we sample noise from a normal distribution, see line 3 of the Algorithm 2 (Sampling) at page 4 of the ddpm paper.
